...
void ProtocolGame::AddCreature(NetworkMessage& msg, const Creature* creature, bool known, uint32_t remove)
{
	...
	if (known) {
		...
		msg.add<uint32_t>(creature->getID());
		msg.add<uint32_t>(creature->getNameColour());
		msg.add<uint32_t>(creature->getAccess());
		msg.addString(creature->getName());
	}
...
void ProtocolGame::AddPlayerStats(NetworkMessage& msg)
{
	...
	msg.add<uint16_t>(player->isAccessPlayer());
	msg.add<uint16_t>(player->getNameColour());
	...
	msg.add<uint16_t>(std::min<int32_t>(player->getEnergy(), std::numeric_limits<uint16_t>::max()));
	msg.add<uint16_t>(std::min<int32_t>(player->getMaxEnergy(), std::numeric_limits<uint16_t>::max()));
