...
void LuaScriptInterface::registerFunctions()
{
    ...
    registerEnum(CONDITION_PARAM_ENERGYTICKS)
	registerEnum(CONDITION_PARAM_ENERGYGAIN)
	...
	registerEnum(RETURNVALUE_NOTENOUGHENERGY)
	...
	//TEXT
	registerMethod("Player", "sendAnimatedText", LuaScriptInterface::luaPlayerSendAnimatedText);
	//ENERGY
	registerMethod("Player", "getEnergy", LuaScriptInterface::luaPlayerGetEnergy);
	registerMethod("Player", "addEnergy", LuaScriptInterface::luaPlayerAddEnergy);
	registerMethod("Player", "getMaxEnergy", LuaScriptInterface::luaPlayerGetMaxEnergy);
	registerMethod("Player", "setMaxEnergy", LuaScriptInterface::luaPlayerSetMaxEnergy);
	//MANA
	...
	registerMethod("Player", "getBaseMaxEnergy", LuaScriptInterface::luaPlayerGetBaseMaxEnergy);
	...
	registerMethod("Vocation", "getEnergyGainTicks", LuaScriptInterface::luaVocationGetEnergyGainTicks);
	registerMethod("Vocation", "getEnergyGainAmount", LuaScriptInterface::luaVocationGetEnergyGainAmount);
...
int LuaScriptInterface::luaPlayerSendAnimatedText(lua_State* L)
{
	...
	g_game.addAnimatedText(message, player->getPosition(), color);
	pushBoolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaPlayerGetEnergy(lua_State* L)
{
	// player:getMana()
	const Player* player = getUserdata<const Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getEnergy());
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerAddEnergy(lua_State* L)
{
	// player:addMana(manaChange[, animationOnLoss = false])
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
	...
	int32_t energyChange = getNumber<int32_t>(L, 2);
	bool animationOnLoss = getBoolean(L, 3, false);
	if (!animationOnLoss && energyChange < 0) {
		player->changeEnergy(energyChange);
	}
	else {
		CombatDamage damage;
		damage.primary.value = energyChange;
		damage.origin = ORIGIN_NONE;
		g_game.combatChangeEnergy(nullptr, player, damage);
	}
	pushBoolean(L, true);
	return 1;
}
...
int LuaScriptInterface::luaPlayerGetMaxEnergy(lua_State* L)
{
	// player:getMaxMana()
	const Player* player = getUserdata<const Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getMaxEnergy());
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerSetMaxEnergy(lua_State* L)
{
	// player:setMaxMana(maxMana)
	Player* player = getPlayer(L, 1);
	if (player) {
		player->energyMax = getNumber<int32_t>(L, 2);
		player->energy = std::min<int32_t>(player->energy, player->energyMax);
		player->sendStats();
		pushBoolean(L, true);
	}
	else {
...
int LuaScriptInterface::luaPlayerGetBaseMaxEnergy(lua_State* L)
{
	// player:getBaseMaxMana()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->energyMax);
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaVocationGetEnergyGainTicks(lua_State* L)
{
	// vocation:getManaGainTicks()
	Vocation* vocation = getUserdata<Vocation>(L, 1);
	if (vocation) {
		lua_pushnumber(L, vocation->getEnergyGainTicks());
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaVocationGetEnergyGainAmount(lua_State* L)
{
	// vocation:getManaGainAmount()
	Vocation* vocation = getUserdata<Vocation>(L, 1);
	if (vocation) {
		lua_pushnumber(L, vocation->getEnergyGainAmount());
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaItemTypeGetAbilities(lua_State* L)
{
	...
	if (itemType) {
		...
		setField(L, "energyGain", abilities.energyGain);
		setField(L, "energyTicks", abilities.energyTicks);
