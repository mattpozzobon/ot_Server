...
class Player final : public Creature, public Cylinder
{
	public:
		...
		const uint16_t getNameColour() const override {
			return nameColour;
		}
		const uint16_t getAccess() const override {
			return nameAccess;
		}
		void setNameColour(const int32_t nameColour) {
			this->nameColour = nameColour;
		}
		void setAccess(const int32_t nameAccess) {
		...
		uint32_t getMaxEnergy() const {
			return std::max<int32_t>(0, energyMax + varStats[STAT_MAXENERGYPOINTS]);
		}
		...
		void changeEnergy(int32_t energyChange);
