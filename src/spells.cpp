...
bool Spell::configureSpell(const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("energy"))) {
		energy = pugi::cast<uint32_t>(attr.value());
	}

	if ((attr = node.attribute("colour"))) {
		colour = pugi::cast<std::string>(attr.value());
	}
...
bool Spell::playerSpellCheck(Player* player) const
{
	...
	if (player->getEnergy() < getEnergyCost() && !player->hasFlag(PlayerFlag_HasInfiniteMana)) {
		player->sendCancelMessage(RETURNVALUE_NOTENOUGHENERGY);
		g_game.addMagicEffect(player->getPosition(), CONST_ME_POFF);
		return false;
	}
...
void Spell::postCastSpell(Player* player, bool finishedCast /*= true*/, bool payCost /*= true*/) const
{
	...
	if (payCost) {
		Spell::postCastSpell(player, getManaCost(player), getEnergyCost(), getSoulCost());
	}
}

void Spell::postCastSpell(Player* player, uint32_t manaCost, uint32_t energyCost, uint32_t soulCost)
{
	if (manaCost > 0) {
	...
	if (energyCost > 0) {
		player->changeEnergy(-static_cast<int32_t>(energyCost));
	}
...
uint32_t Spell::getEnergyCost() const
{
	if (energy != 0) {
		return energy;
	}
