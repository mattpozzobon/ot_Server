...
bool Vocations::loadFromXml()
{
	...
	for (auto vocationNode : doc.child("vocations").children()) {
		...
		for (auto attrNode : vocationNode.attributes()) {
			...
			else if (strcasecmp(attrName, "gainenergyticks") == 0) {
				voc.gainEnergyTicks = pugi::cast<uint32_t>(attrNode.value());
			}
			else if (strcasecmp(attrName, "gainenergyamount") == 0) {
				voc.gainEnergy = pugi::cast<uint32_t>(attrNode.value());
			}
