...
bool Game::playerSaySpell(Player* player, SpeakClasses type, const std::string& text)
{
	...
	if (result == TALKACTION_BREAK) {
		if (!g_config.getBoolean(ConfigManager::EMOTE_SPELLS)) {
			...
			TextColor_t colour = g_spells->getInstantSpell(words)->getColour();
			addAnimatedText(words, player->position, colour);
			return true;
		}
...
bool Game::internalCreatureSay(Creature* creature, SpeakClasses type, const std::string& text, bool ghostMode, SpectatorVec* spectatorsPtr/* = nullptr*/, const Position* pos/* = nullptr*/)
{
	if (text.empty()) {
		return false;
	}
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (manaChange > 0) {
		...
		TextMessage message;
		std::ostringstream strEnergyLoss;
		strEnergyLoss << manaLoss;
		addAnimatedText(strEnergyLoss.str(), targetPos, TEXTCOLOR_BLUE);
...
bool Game::combatChangeEnergy(Creature* attacker, Creature* target, CombatDamage& damage)
{
	Player* targetPlayer = target->getPlayer();
	if (!targetPlayer) {
		return true;
	...
	int32_t energyChange = damage.primary.value + damage.secondary.value;
	if (energyChange > 0) {
		if (attacker) {
			const Player* attackerPlayer = attacker->getPlayer();
			if (attackerPlayer && attackerPlayer->getSkull() == SKULL_BLACK && attackerPlayer->getSkullClient(target) == SKULL_NONE) {
		...
		if (damage.origin != ORIGIN_NONE) {
			...
			if (!events.empty()) {
				for (CreatureEvent* creatureEvent : events) {
					creatureEvent->executeEnergyChange(target, attacker, damage);
				}
				damage.origin = ORIGIN_NONE;
				return combatChangeEnergy(attacker, target, damage);
			}
		}

		int32_t realEnergyChange = targetPlayer->getEnergy();
		targetPlayer->changeEnergy(energyChange);
		realEnergyChange = targetPlayer->getEnergy() - realEnergyChange;
	}
	else {
		const Position& targetPos = target->getPosition();
		if (!target->isAttackable()) {
		...
		int32_t energyLoss = std::min<int32_t>(targetPlayer->getEnergy(), -energyChange);
		BlockType_t blockType = target->blockHit(attacker, COMBAT_MANADRAIN, energyLoss);
		if (blockType != BLOCK_NONE) {
			addMagicEffect(targetPos, CONST_ME_POFF);
			return false;
		}
		...
		targetPlayer->drainMana(attacker, energyLoss);

		std::string spectatorMessage;

		TextMessage message;
		std::ostringstream strManaLoss;
		strManaLoss << energyLoss;
		...
		for (Creature* spectator : spectators) {
			...
			if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
				message.type = MESSAGE_STATUS_DEFAULT;
				message.text = fmt::format("{:s} loses {:d} mana due to your attack.", target->getNameDescription(), energyLoss);
				message.text[0] = std::toupper(message.text[0]);
			}
			else if (tmpPlayer == targetPlayer) {
				message.type = MESSAGE_STATUS_DEFAULT;
				if (!attacker) {
					message.text = fmt::format("You lose {:d} mana.", energyLoss);
				}
				else if (targetPlayer == attackerPlayer) {
					message.text = fmt::format("You lose {:d} mana due to your own attack.", energyLoss);
				}
				else {
					message.text = fmt::format("You lose {:d} mana due to an attack by {:s}.", energyLoss, attacker->getNameDescription());
				}
			}
			else {
				message.type = MESSAGE_STATUS_DEFAULT;
				if (spectatorMessage.empty()) {
					if (!attacker) {
						spectatorMessage = fmt::format("{:s} loses {:d} mana.", target->getNameDescription(), energyLoss);
					}
					else if (attacker == target) {
						spectatorMessage = fmt::format("{:s} loses {:d} mana due to {:s} own attack.", target->getNameDescription(), energyLoss, targetPlayer->getSex() == PLAYERSEX_FEMALE ? "her" : "his");
					}
					else {
						spectatorMessage = fmt::format("{:s} loses {:d} mana due to an attack by {:s}.", target->getNameDescription(), energyLoss, attacker->getNameDescription());
					}
					spectatorMessage[0] = std::toupper(spectatorMessage[0]);
				}
			}
