...
void ConditionRegeneration::addCondition(Creature*, const Condition* condition)
{
	if (updateCondition(condition)) {
		...
		energyTicks = conditionRegen.energyTicks;
		energyGain = conditionRegen.energyGain;
	}
}
...
bool ConditionRegeneration::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	...
	} else if (attr == CONDITIONATTR_ENERGYGAIN) {
		return propStream.read<uint32_t>(energyGain);
	} else if (attr == CONDITIONATTR_ENERGYTICKS) {
		return propStream.read<uint32_t>(energyTicks);
	}
...
void ConditionRegeneration::serialize(PropWriteStream& propWriteStream)
{
	...
	propWriteStream.write<uint8_t>(CONDITIONATTR_ENERGYTICKS);
	propWriteStream.write<uint32_t>(energyTicks);

	propWriteStream.write<uint8_t>(CONDITIONATTR_ENERGYGAIN);
	propWriteStream.write<uint32_t>(energyGain);
}
...
bool ConditionRegeneration::executeCondition(Creature* creature, int32_t interval)
{
	...
	if (internalEnergyTicks >= energyTicks) {
		internalEnergyTicks = 0;

		if (Player* player = creature->getPlayer()) {
			int32_t realEnergyGain = player->getEnergy();

			player->changeEnergy(energyGain);
			realEnergyGain = player->getEnergy() - realEnergyGain;

			if (isBuff && realEnergyGain > 0) {
				std::string energyGainString = std::to_string(realEnergyGain);

				TextMessage message(MESSAGE_STATUS_DEFAULT, "You gained " + energyGainString + " energy.");
				player->sendTextMessage(message);

				g_game.addAnimatedText(std::to_string(realEnergyGain), player->getPosition(), TEXTCOLOR_YELLOW);
				...
				if (!spectators.empty()) {
					message.type = MESSAGE_STATUS_DEFAULT;
					message.text = player->getName() + " gained " + energyGainString + " energy.";
					for (Creature* spectator : spectators) {
						spectator->getPlayer()->sendTextMessage(message);
					}
				}
...
bool ConditionRegeneration::setParam(ConditionParam_t param, int32_t value)
{
	...
	switch (param) {
		...
		case CONDITION_PARAM_ENERGYGAIN:
			energyGain = value;
			return true;

		case CONDITION_PARAM_ENERGYTICKS:
			energyTicks = value;
			return true;
