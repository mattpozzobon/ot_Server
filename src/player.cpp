...
void Player::setVarStats(stats_t stat, int32_t modifier)
{
	...
	switch (stat) {
		...
		case STAT_MAXENERGYPOINTS: {
			if (getEnergy() > getMaxEnergy()) {
				changeEnergy(getMaxEnergy() - getEnergy());
			}
			break;
		}
...
int32_t Player::getDefaultStats(stats_t stat) const
{
	switch (stat) {
		...
		case STAT_MAXENERGYPOINTS: return energyMax;
...
void Player::changeEnergy(int32_t energyChange)
{

	if (energyChange > 0) {
		energy += std::min<int32_t>(energyChange, getMaxEnergy() - energy);
	}
	else {
		energy = std::max<int32_t>(0, energy + energyChange);
	}
...
void Player::updateRegeneration()
{
	...
	if (condition) {
		...
		condition->setParam(CONDITION_PARAM_ENERGYGAIN, vocation->getEnergyGainAmount());
		condition->setParam(CONDITION_PARAM_ENERGYTICKS, vocation->getEnergyGainTicks() * 1000);
	}
}
